...
class RedactCharacterFragment : Fragment() {
    ...
    Bundle?): View {
        ...
        val thr = Thread(kotlinx.coroutines.Runnable {
            ...
            requireActivity().runOnUiThread {
                ...
                val adapterClass: ArrayAdapter<String> = ArrayAdapter<String>(
                    requireContext(),
                    R.layout.simple_spinner_item, arrClassName
                )
                val adapterRace: ArrayAdapter<String> = ArrayAdapter<String>(
                    requireContext(),
                    R.layout.simple_spinner_item, arrRaceName
                )
        ...
        pict.setText(character.pictID)
        notes.setText(character.notes)
        health.setText(character.hp.toString())
        power.setText(character.power.toString())
        agility.setText(character.agility.toString())
        bodyType.setText(character.bodyType.toString())
        wisdom.setText(character.wisdom.toString())
        charisma.setText(character.charisma.toString())
        intellect.setText(character.charisma.toString())
        ...
        btnClass.setOnClickListener {
            ...
            val thr2 = Thread(kotlinx.coroutines.Runnable {
                ...
                val adapterWeap: ArrayAdapter<String> = ArrayAdapter<String>(
                    requireContext(),
                    R.layout.simple_spinner_item, arrWeapName
                )
                val adapterArm: ArrayAdapter<String> = ArrayAdapter<String>(
                    requireContext(),
                    R.layout.simple_spinner_item, arrArmName
                )
                requireActivity().runOnUiThread {
        ...
        btnRace.setOnClickListener {
            ...
            val thr2 = Thread(kotlinx.coroutines.Runnable {
                ...
                val adapterVarRace: ArrayAdapter<String> = ArrayAdapter<String>(
                    requireContext(),
                    R.layout.simple_spinner_item, arrVarRaceName
                )
                requireActivity().runOnUiThread {
        ...
        saveBtn.setOnClickListener {
            ...
            if(spinArm.adapter == null)
                bool = false
            if(bool) {
                //формируем исправленного перса и отправляем
                val charact = CharacterData(character.accID, agility.text.toString().toLong(), allArm[
                    ...
                    charisma.text.toString().toLong(), allClasses[spinClass.selectedItemId.toInt()]
                        .id, false, character.gameID, health.text.toString().toLong(), false,
                    character.id, intellect.text.toString().toLong(), name.text.toString(), notes.text.toString(),
                ...
                val thr2 = Thread(kotlinx.coroutines.Runnable {
                    netHelper.redactCharacter(charact.toJson())
                })
                thr2.start()
                Thread.sleep(1500)
    ...
    companion object {
        ...
        fun newInstance(character: CharacterData) =
            RedactCharacterFragment().apply {
                arguments = Bundle().apply {
                    putSerializable(ARG_PARAM1, character)
                }
            }
    }
}